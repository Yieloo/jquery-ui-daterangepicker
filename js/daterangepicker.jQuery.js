...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			//{text: 'N\'importe quand', dateStart: '', dateEnd: ''},
			{text: 'Aujourd\'hui', dateStart: 'today', dateEnd: 'today' },
			{text: 'La semaine passée', dateStart: 'today-7days', dateEnd: 'today' },
			{text: 'Le mois courant', dateStart: function(){ return Date.parse('today').moveToFirstDayOfMonth();  }, dateEnd: function(){ return Date.parse('today').moveToLastDayOfMonth();  } },
			{text: 'L\'année courante', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today' },
			//extras:
		...
		presets: {
			specificDate: 'A la date du', 
			allDatesBefore: 'Avant le', 
			allDatesAfter: 'Après le', 
			dateRange: 'Durant cet intervalle'
		},
